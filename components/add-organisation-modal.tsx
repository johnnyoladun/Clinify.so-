...
import { X, Building2, Mail, MapPin, FileText, ArrowRight } from 'lucide-react'
...
export function AddOrganisationModal({ isOpen, onClose, onSuccess }: AddOrganisationModalProps) {
  ...
  const [formId, setFormId] = useState('') // Extracted from table
  ...
  const fetchFormFields = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormId(data.formId || '') // Store the extracted form ID
        setStep(3)
      } else {
        const data = await res.json()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Trigger immediate sync from Jotform
        console.log('Organisation created, triggering sync...')
        try {
  ...
  const getStepTitle = () => {
    if (step === 1) return 'Organisation Details'
    if (step === 2) return 'Location & Table ID'
    return 'Map Form Fields'
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={handleClose}>
      <div className="relative w-full max-w-3xl rounded-lg border border-border bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between border-b border-border p-6">
          <div>
            <h3 className="text-xl font-semibold">Add New Organisation</h3>
        ...
        <div className="p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., SATIVA, Cannafrica, etc."
          ...
          {/* Step 3: Field Mapping - Drag and Drop */}
          {step === 3 && (
            <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div
                      ...
                      className={`rounded-lg border-2 border-dashed p-4 transition-all ${
                        fieldMapping.patientName
                          ...
                          : 'border-border bg-black/10 hover:border-blue-400 hover:bg-blue-500/5'
                      }`}
                    >
                      ...
                      {fieldMapping.patientName ? (
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium">
                            {formFields.find(f => f.id === fieldMapping.patientName)?.text || 'Mapped'}
                          </p>
                          <button
                            onClick={() => setFieldMapping({ ...fieldMapping, patientName: '' })}
                    ...
                    <div
                      ...
                      className={`rounded-lg border-2 border-dashed p-4 transition-all ${
                        fieldMapping.idDocument
                          ? 'border-green-500 bg-green-500/5'
                          : 'border-border bg-black/10 hover:border-yellow-400 hover:bg-yellow-500/5'
                      }`}
                    >
                      ...
                      {fieldMapping.idDocument ? (
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium">
                            {formFields.find(f => f.id === fieldMapping.idDocument)?.text || 'Mapped'}
                          </p>
                          <button
                            onClick={() => setFieldMapping({ ...fieldMapping, idDocument: '' })}
                            className="text-xs text-red-400 hover:text-red-300"
                          >
                            Remove
                          </button>
                    ...
                    <div
                      ...
                      className={`rounded-lg border-2 border-dashed p-4 transition-all ${
                        fieldMapping.drScript
                          ? 'border-green-500 bg-green-500/5'
                          : 'border-border bg-black/10 hover:border-green-400 hover:bg-green-500/5'
                      }`}
                    >
                      ...
                      {fieldMapping.drScript ? (
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium">
                            {formFields.find(f => f.id === fieldMapping.drScript)?.text || 'Mapped'}
                          </p>
                          <button
                            onClick={() => setFieldMapping({ ...fieldMapping, drScript: '' })}
                            className="text-xs text-red-400 hover:text-red-300"
                          >
                            Remove
                          </button>
                    ...
                    >
                      ...
                      {fieldMapping.outcomeLetters ? (
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium">
                            {formFields.find(f => f.id === fieldMapping.outcomeLetters)?.text || 'Mapped'}
                          </p>
                          <button
                            onClick={() => setFieldMapping({ ...fieldMapping, outcomeLetters: '' })}
                            className="text-xs text-red-400 hover:text-red-300"
                          >
                            Remove
                          </button>
